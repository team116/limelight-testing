...
public class Robot extends TimedRobot {
  ...
  public void teleopPeriodic() {
    double currDist = m_robotContainer.getDistanceFromAprilTagInches();
    double currDeg = m_robotContainer.getHorizontalDegreeOffset();
    
    if(currDist != Limelight.getDistanceFromAprilTagInches(1.0d) && currDist != lastDist) {
      lastDist = currDist;
    }

    if(currDeg != 1.0d && currDeg != lastDegree) {
      lastDegree = currDeg;
    }
    ...
    SmartDashboard.putNumber("Vertical Offset from April Tag (deg.)", m_robotContainer.limelight.getTy());
